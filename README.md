...
脚本会优先安装并运行 Ookla 官方 `speedtest`，要求用户在中国移动、中国联通、中国电信中选择一个运营商，然后优先尝试中国境内最近的匹配测速节点并获取上传带宽、下载带宽和 Ping；如果找不到匹配节点，会降级为中国境内不限运营商节点，最后才使用 Ookla 自动选择。测速失败时会提示手动输入上传带宽。
